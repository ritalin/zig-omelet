...
pub fn ConnectPhaseState(comptime GuestStage: type) type {
    return struct {
        const Self = @This();
        ...
        pub fn handle(self: *const Self, stage: *GuestStage, entry: ReceiveEntry, dirty: *EventDispatcher.DirtyState) !void {
            switch (entry.event) {
                .probe => |phase| {
                    if ((phase == .connecting) and (std.meta.eql(stage.dispatcher.phase, .{.kind = .connecting, .agreement = .pending}))) {
                        // Response by RPC
        ...
        fn connectLog(self: *const Self, stage: *GuestStage) !void {
            _ = self;
