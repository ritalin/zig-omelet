    ...
    loop: while (true) {
        ...
        while (it.next()) |item| {
            ...
            switch (ev) {
                .begin_topic => {
                    topics: {
                        try core.sendEventWithPayload(self.allocator, self.sender_socket, .topic, &[_]core.Symbol{"query"});
                        try core.sendEventWithPayload(self.allocator, self.sender_socket, .topic, &[_]core.Symbol{"name"});
                        try core.sendEventWithPayload(self.allocator, self.sender_socket, .topic, &[_]core.Symbol{"placeholder"});
                        break :topics;
                    }
                    ack_topics: {
                        try core.sendEvent(managed_allocator, self.sender_socket, .end_topic);
                        break :ack_topics;
                    }
