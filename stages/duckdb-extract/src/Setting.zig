...
schema_dir_set: []const core.types.FilePath,
...
pub fn loadFromArgs(allocator: std.mem.Allocator, args: std.process.Args) core.settings.types.LoadResult(Setting, *const ArgHelp.Config) {
    ...
    var scanner: ArgScanner(std.process.Args.Iterator) = .init(&iter);
...
pub fn deinit(self: *Setting, allocator: std.mem.Allocator) void {
    ...
    for (self.schema_dir_set) |path| {
        allocator.free(path);
    }
    allocator.free(self.schema_dir_set);
}
...
fn Builder(comptime ArgIterator: type) type {
    return struct {
        ...
        pub fn fromArgs(allocator: std.mem.Allocator, scanner: *ArgScanner(ArgIterator), log_style: core.Logger.LogStyle) !Builder(ArgIterator) {
            ...
            var parsers = ArgParserPair(GuestBaseConfigArgId, GuestExtractArgId, ArgIterator).init(scanner, &diag);
        ...
        fn handleExtraArg(self: *Builder(ArgIterator), allocator: std.mem.Allocator, arg: clap.streaming.Arg(GuestExtractArgId)) !void {
            switch (arg.param.id) {
        ...
        pub fn build (self: Builder(ArgIterator), allocator: std.mem.Allocator) !Setting {
            ...
            var schemas: std.ArrayListUnmanaged(FilePath) = .empty;
            defer schemas.deinit(allocator);
            for (self.schema_dir_set.items) |path_| {
                if (path_) |path| {
                    try schemas.append(allocator, try allocator.dupe(u8, path));
                }
            }
            ...
            return .{
                ...
                .schema_dir_set = try schemas.toOwnedSlice(allocator),
            };
        }
    };
}